...
Процедура ГСД_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	Если ВключитьПоказСуммВыделенныхСтрок Тогда              
		...
		Расчет_Сервер.ДобавитьРеквизитыИЭлементыФормы(ЭтотОбъект, Элементы.ГруппаИтоговПоДокументу,ИмяРеквизитаСРезультатомСуммирования);
		
		//подключаем контекстное меню к табличным частям
		ИмяТаблицы="Начисления";
		Расчет_Сервер.ДополнитьКонтекстноеМеню(ЭтотОбъект,ИмяТаблицы,"ГСД_РассчитатьСуммуВыделенныхСтрокНачисления");
		
		ИмяТаблицы="НДФЛ";
		Расчет_Сервер.ДополнитьКонтекстноеМеню(ЭтотОбъект,ИмяТаблицы,"ГСД_РассчитатьСуммуВыделенныхСтрокНДФЛ");

		ИмяТаблицы="Удержания";
		Расчет_Сервер.ДополнитьКонтекстноеМеню(ЭтотОбъект,ИмяТаблицы,"ГСД_РассчитатьСуммуВыделенныхСтрокУдержания");
...
Процедура ГСД_РассчитатьСуммуВыделенныхСтрокНачисления()   
	Если ВключитьПоказСуммВыделенныхСтрок Тогда	
		Расчет_Клиент.РассчитатьСуммуВыделенныхСтрок(ЭтаФорма,"Начисления","Результат",ИмяРеквизитаСРезультатомСуммирования); 
	КонецЕсли;	
...
Процедура ГСД_РассчитатьСуммуВыделенныхСтрокНДФЛ()
	Если ВключитьПоказСуммВыделенныхСтрок Тогда	
		Расчет_Клиент.РассчитатьСуммуВыделенныхСтрок(ЭтаФорма,"НДФЛ","Налог",ИмяРеквизитаСРезультатомСуммирования); 
	КонецЕсли;		
КонецПроцедуры
...
Процедура ГСД_РассчитатьСуммуВыделенныхСтрокУдержания()
	Если ВключитьПоказСуммВыделенныхСтрок Тогда	
		Расчет_Клиент.РассчитатьСуммуВыделенныхСтрок(ЭтаФорма,"Удержания","Результат",ИмяРеквизитаСРезультатомСуммирования); 		
	КонецЕсли;		
